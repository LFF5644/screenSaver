...
function createProgressbar(
	...
	text._content=text.content.split("%p").join(String(progress.value)); // "Update %p% Abgeschlossen" => "Update 95% Abgeschlossen"

	const [lengthX,lengthY]=getTextLength(text.size,text._content);

	text.id=writeText(pStartX+(pWidth-lengthX)/2,pStartY+(pHeight-lengthY)/2,text.size,text._content,...text.color);

	delete text._content;
...
function changeProgress(id,percent){
	...
	text._content=text.content.split("%p").join(String(percent)); // "Update %p% Abgeschlossen" => "Update 95% Abgeschlossen"
	const [lengthX,lengthY]=getTextLength(text.size,text._content);
	progressbar.text.id=writeText(pStartX+(pWidth-lengthX)/2,pStartY+(pHeight-lengthY)/2,text.size,text._content,...text.color);

	displayedProgressbar.set(id,progressbar);
}
