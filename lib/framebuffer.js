...
function getTextLength(size,content){
	...
	for(let index=0; index<content.length; index+=1){
		...
		if(!charObject) throw new Error("char "+char+" do not exist! add to characters.json & build first!");
...
function createProgressbar(
	...
	if(text.onUpdate){
		text.currentContent=text.onUpdate(progress.value);
	}
...
function removeProgressbar(id,...rgb){
	const progressbar=displayedProgressbar.get(id);
	if(!progressbar) throw new Error("progressbar id "+id+" not exist");

	const {startX,startY,width,height}=progressbar;

	if(!rgb.length===3) rgb=bgColor;
	writeRectangle(startX,startY,width,height,...rgb);
	displayedProgressbar.delete(id);
}
